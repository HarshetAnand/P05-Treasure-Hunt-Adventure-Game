...
//
// Title: P05 Treasure Hunt: TreasureHunt Class
...
/**
 * This class models the PApplet display window of the cs300 spring 2022 p05 Treasure Hunt adventure
 * style game.
 */
...
public class TreasureHunt extends PApplet {
  ...
  public void initGame() {
    ...
    ScreenshotButton takeScreenshotButton = new ScreenshotButton(140, 0);
    this.gameObjects.add(takeScreenshotButton);
  ...
  public void draw() {
    ...
    for (int j = 0; j < gameObjects.size(); j++) {
      gameObjects.get(j).draw();
    }

    // TODO
    // 3. traverse the list of gameObjects and remove deactivated interactive objects
    for (int j = 0; j < gameObjects.size(); j++) {
      if (gameObjects.get(j) instanceof InteractiveObject) {
        if (((InteractiveObject) gameObjects.get(j)).isActive() != true) {
  ...
  public void mousePressed() {
    ...
    // the list
    for (int j = 0; j < gameObjects.size(); j++) {
      gameObjects.get(j).mousePressed();
    }
  }
   ...
   * 
   * @param name of object being found
   * @return reference to an interactive object with the specified name, or null when none is found
   */
  protected InteractiveObject findObjectByName(String name) {
    for (int j = 0; j < gameObjects.size(); j++) {
      if (gameObjects.get(j) instanceof InteractiveObject) {
        if (((InteractiveObject) gameObjects.get(j)).hasName(name)) {
          return (InteractiveObject) gameObjects.get(j);
        }
      }
    }
    return null;
